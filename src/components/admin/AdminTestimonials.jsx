...
function TestimonialCard({ t, onToggle, onDelete }) {
  return (
    <div className={`p-4 rounded-xl border bg-card transition-colors ${t.approved ? "border-primary/20" : "border-border"}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="font-medium text-sm">{t.author_name}</p>
          <p className="text-xs text-muted-foreground mb-2">{t.author_role}</p>
