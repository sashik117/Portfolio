...
const FALLBACK_PROJECTS = [
  {
    ...
    desc_uk: "Повна автоматизація бізнес-процесів дилерського центру. Зв'язав сайт на WordPress із 1С та CRM-системами Webra та Uspacy. Використав n8n для складних сценаріїв передачі даних між сервісами в реальному часі.",
    desc_en: "Full automation of dealership business processes. Connected a WordPress site with 1C and CRM systems Webra and Uspacy. Used n8n for complex real-time data transfer scenarios between services.",
    tags: ["PHP", "Node.js", "n8n", "REST API", "Webhooks"],
    color: "from-primary/20 to-accent/20",
    categories: ["Full-Stack", "Automation"],
    live_url: "https://github.com/stewe-snowwhite",
    github_url: "https://github.com/stewe-snowwhite",
  },
  {
    id: "f2",
    title: "Automated Sales Funnel",
    desc_uk: "Система автоматичного збору лідів з сайту та їх розподілу в CRM через n8n з інтеграцією месенджерів. Повністю виключила ручне опрацювання вхідних заявок.",
    desc_en: "Automatic lead collection system from the website and distribution to CRM via n8n with messenger integration. Completely eliminated manual processing of incoming requests.",
...
function ProjectCard({ project, viewLabel }) {
  ...
  const desc = lang === "uk" ? project.desc_uk : project.desc_en;
  return (
        ...
        <motion.div layout className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          <AnimatePresence mode="popLayout">
            {filtered.map((project) => (
              <ProjectCard key={project.id || project.title} project={project} viewLabel={p.view} />
            ))}
          </AnimatePresence>
        </motion.div>
        {filtered.length === 0 && (
