...
const SAMPLE_TESTIMONIALS = [
  ...
  },
  {
    ...
    message: "Працював зі Stewe як з молодшим розробником на початку його шляху. Він дуже швидко схоплює нові концепції, не боїться складних завдань і завжди доводить справу до кінця.",
    rating: 5,
  },
];
    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <button key={star} type="button" onClick={() => onChange(star)}
          ...
          <Star className={`w-7 h-7 transition-colors ${star <= (hovered || value) ? "text-chart-4 fill-chart-4" : "text-border"}`} />
        </button>
      ))}
    </div>
  );
...
export default function TestimonialsSection() {
  ...
  const [submitStatus, setSubmitStatus] = useState(null); // 'ok' | 'err' | null

  useEffect(() => {
    supabase.from("testimonials").select("*").eq("approved", true).then(({ data, error }) => {
          ...
          <div className="relative rounded-2xl bg-card border border-border p-8 sm:p-12 overflow-hidden min-h-[280px] flex flex-col justify-between">
            ...
            <AnimatePresence custom={direction} mode="wait">
              <motion.div key={current} custom={direction} variants={variants}
                ...
                <p className="text-lg sm:text-xl text-foreground leading-relaxed mb-8 italic">"{item.message}"</p>
